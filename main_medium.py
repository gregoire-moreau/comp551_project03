...
data = np.zeros((40000, 784), dtype=np.float32)
...
eval_data = data[32000:]
eval_labels = np.asarray(train_classes.iloc[32000:, 1], dtype=np.int32)
...
dataTest = np.zeros((len(test_images), 784), dtype=np.float32)
for i in range(len(test_images)):
    res = cv2.resize(train_images[i], dsize=(28,28), interpolation=cv2.INTER_CUBIC)
    dataTest[i] = np.reshape(res, (784,))


predictions = mnist_classifier.predict(
    input_fn=tf.estimator.inputs.numpy_input_fn(x={"x": dataTest},
                                                num_epochs=1,
                                                shuffle=False)
)
